...
public class Room extends BaseObject {
	...
	private void createBody() {
		...
		roomPoints = Algorithms.MakingCircle.useTrigonometry(Const.OBSTACLE_TOTAL, initPos,
				Const.SPIKE_RADIUS / Const.PPM);
		...
		defBd.setFixtureDef(chain, Const.ROOM_DENSITY, Const.ROOM_FRICTION, Const.ROOM_RESTITUTION);
		...
		staticChain.createLoop((Vector2[]) Algorithms.MakingCircle
				.useTrigonometry(Const.OBSTACLE_TOTAL, initPos, (Const.SPIKE_RADIUS - 6) / Const.PPM)
				.toArray(Vector2.class));
		defBd.bdef.type = BodyType.StaticBody;
		defBd.setFixtureDef(staticChain, Const.ROOM_DENSITY, Const.ROOM_FRICTION, Const.ROOM_RESTITUTION);
		defBd.fdef.filter.categoryBits = Const.BIT_ROOM;
		defBd.fdef.filter.maskBits = Const.BIT_FROG | Const.BIT_FROG_CLONER | Const.BIT_METEOR;
	...
	public void update(float delta) {
		for (int i = 0; i < roomPoints.size; i++) {
			ChainShape chain = (ChainShape) body.getFixtureList().first().getShape();
